...
<div class="max-w-2xl">
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div class="px-6 py-4 bg-slate-50 border-b border-slate-100">
            <p class="text-sm text-slate-600 leading-relaxed">{{ $tugas->deskripsi }}</p>
        </div>
        @endif
        ...
        <form action="{{ route('mahasiswa.tugas.submit', $tugas) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
            ...
            <div class="p-4 bg-red-50 border border-red-200 rounded-xl">
                <ul class="space-y-1">
                    ...
                    <li class="text-sm text-red-700 flex items-center gap-1.5"><span class="material-symbols-outlined text-sm">error</span>{{ $error }}</li>
                    @endforeach
                </ul>
            </div>
            @endif
            ...
            <div class="p-4 bg-amber-50 border border-amber-200 rounded-xl text-sm text-amber-800">
                ...
                Upload ulang akan menggantikan file sebelumnya. Dikumpulkan pada: {{ $pengumpulan->dikumpulkan_at->isoFormat('D MMM Y, HH:mm') }}
            </div>
            @endif

            {{-- Upload --}}
            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-2">File Tugas <span class="text-red-500">*</span></label>
                <div class="relative border-2 border-dashed border-slate-300 rounded-xl bg-slate-50 hover:border-primary hover:bg-primary/5 transition-all cursor-pointer"
                     id="dropzone">
            ...
            <div>
                ...
                <textarea name="catatan" rows="3"
                          placeholder="Tambahkan catatan untuk supervisor Anda..."
            ...
            <div class="flex gap-3 pt-2">
                ...
                <a href="{{ route('mahasiswa.tugas.index') }}" class="flex items-center gap-2 border border-slate-300 text-slate-700 font-bold py-3 px-6 rounded-xl hover:bg-slate-50 transition-all text-sm">
                    Batal
                </a>
            </div>
        </form>
